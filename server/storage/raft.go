...
func newRaftNode(id int, storage string, peers []string, getSnapshot func() ([]byte, error), proposeC <-chan string) (<-chan *commit, <-chan error, <-chan *snap.Snapshotter) {

	commitC := make(chan *commit)
	errorC := make(chan error)

	waldir := fmt.Sprintf("server/storage/data/%sserver-%d", storage, id)
	snapdir := fmt.Sprintf("server/storage/data/%sserver-%d-snap", storage, id)

	if storage == "test" {
		waldir = fmt.Sprintf("data/%sserver-%d", storage, id)
		snapdir = fmt.Sprintf("data/%sserver-%d-snap", storage, id)
	}
